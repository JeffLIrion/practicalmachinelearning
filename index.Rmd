...
# figure out which predictors from testdata don't match with traindata
trainfactors <- names(traindata)[ sapply(traindata, is.factor)]
testfactors <- names(testdata)[ sapply(testdata, is.factor)]
levels(testdata[[testfactors[1]]]) <- levels(traindata[[trainfactors[1]]])
...
# columns to potentially be excluded from the features
exclude <- c(1,2,3,4,5,6,7,160)

folds <- createFolds(traindata$classe)

traindata2 <- traindata[inTrain,]
traindata2 <- traindata[1:10,]
traindata2 <- traindata[,c(1,160)]
traindata2 <- traindata[1:100,]
inTrain2 <- createDataPartition(traindata$classe, p=0.1, list=FALSE)
traindata2 <- traindata[inTrain2,]
na_rows <- apply(traindata, 1, function(x) sum(is.na(x)))
na_cols <- apply(traindata, 2, function(x) sum(is.na(x)))
num_cols <- apply(traindata, 2, function(x) sum(is.numeric(x)))
traindata2 <- traindata[,!na_cols]
comps <- complete.cases(t(traindata)) & complete.cases(t(testdata))
traindata2 <- traindata[,comps]

# fit a model
modFit1 <- train(classe~., data=traindata2, method="rf", na.action=na.exclude)
modFit2 <- randomForest(classe~., data=traindata2)
```
